...
class HtmlToListOfDictConverter:
    ...
    def clear_white_characters(self, text: str):
        return text.strip().replace('\n', '').replace('\t', '').replace('\xa0', ' ').replace('  ', '')
    
    def create_offert_dict(self, item_div_elements: BeautifulSoup):
        offert_info = {}
        try:
            price = self.clear_white_characters(item_div_elements.select_one(select_offert.get_price()).text)
            if price == 'Zapytaj o cenę':
                return offert_info
            offert_title = self.clear_white_characters(item_div_elements.select_one(select_offert.get_offert_title()).text)
            location_information = self.clear_white_characters(item_div_elements.select_one(select_offert.get_location_information()).text)
            surface = self.clear_white_characters(item_div_elements.select_one(select_offert.get_surface()).find(select_offert.get_surface_label(), text=select_offert.get_surface_label_text()).find_next_sibling(select_offert.get_surface_value()).text)
            rooms= self.clear_white_characters(item_div_elements.select_one(select_offert.get_rooms()).find(select_offert.get_rooms_label(), text=select_offert.get_rooms_label_text()).find_next_sibling(select_offert.get_rooms_value()).text)
